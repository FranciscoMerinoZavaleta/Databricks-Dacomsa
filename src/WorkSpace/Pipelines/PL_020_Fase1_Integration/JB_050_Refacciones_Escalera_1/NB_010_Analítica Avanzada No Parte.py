...
pedidos_productos = (fact_pedidos.groupBy('MES_FECHA_REGISTRO',
 'ANIO_FECHA_REGISTRO',
 'ID_MATERIAL',
 'ANIO_MES',
 'ID_CANAL_DISTRIBUCION',"FECHA_REGISTRO").agg(  
...
pedidos_productos = pedidos_productos.drop('FECHA_REGISTRO')
...
pedidos_productos = pedidos_productos.join(dim_productos_materiales,
                                                   on = ["ID_MATERIAL","ID_CANAL_DISTRIBUCION"],
                                                   how = "inner")

# COMMAND ----------

pedidos_productos = pedidos_productos.withColumn("ANIO_MES_1", F.col("ANIO_MES")) \
                                   .withColumn("ID_CANAL_DISTRIBUCION_1", F.col("ID_CANAL_DISTRIBUCION"))

# COMMAND ----------

pedidos_productos = pedidos_productos.withColumn("ID_PRD", F.concat(F.col("ID_MATERIAL"), F.lit("-"), F.col("ID_CANAL_DISTRIBUCION")))

# COMMAND ----------

fechas = pedidos_productos.select("ANIO_MES").distinct()
# fechas.display()
...
combinaciones = pedidos_productos.select("ID_PRD").distinct()

# COMMAND ----------

combinaciones_prd = pedidos_productos.select("ID_PRD","ID_MATERIAL","ID_CANAL_DISTRIBUCION").distinct()

# COMMAND ----------

combinaciones_prd = combinaciones_prd.withColumnRenamed("ID_PRD","Combinacion")
...
pedidos_productos = combinaciones_fechas.join(pedidos_productos,
                          on =  ["ANIO_MES","ID_PRD"],
                          how = "left")

# COMMAND ----------

df_modelo = pedidos_productos
...
print("Combinaciones a modelar: ", len(combinaciones_a_modelar))
for valor in range(len(combinaciones_a_modelar)):
    print("Combinaciones:",valor)
...
print("Part2 mape_value_total_aph_no_parte_total")
mape_value_aph_total = spark.createDataFrame(mape_value_total)
table = "mape_value_total_aph_no_parte_total"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
(mape_value_aph_total.
    ...
    mode("overwrite").
    parquet(target_path))
print("Part3 mape_value_total_aph_no_parte_total")
...
table = "resultados_aph_no_parte_total"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
...
table = "resultado_modelos_no_modelables_no_parte_total"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
...
# resultados_ap_total= resultados_ap_total.withColumn("ANIO_MES_1",col("ANIO_MES"))
table = "modelos_no_modelables_no_parte_total"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
...
resultados_pronostico = resultados_pronostico.join(combinaciones_prd, on = 'Combinacion', how = 'inner')
...
table = "resultados_pronostico_no_parte"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
...
table = "resultados_pronostico_no_parte_escalera"
target_path = f"/mnt/adls-dac-data-bi-pr-scus/gold/analitica_avanzada/{table}.parquet/"
#Escritura de los archivos gold
(resultados_pronostico.
    write.format('delta ').
