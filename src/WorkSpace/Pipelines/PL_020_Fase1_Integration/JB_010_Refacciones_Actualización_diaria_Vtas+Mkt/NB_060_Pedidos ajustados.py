...
# MAGIC %md
# MAGIC # Extracción Pedidos Escalera
...
# fact_pedidos.filter(col("ID_MATERIAL")=='MU-41420-E-D').display()
...
fact_pedidos.filter(col("ID_MATERIAL")=='MU-41420-E-D').groupBy("ANIO_MES","ANIO_MES_PEDIDO").agg(sum("IMPORTE"),sum("IMPORTE_RECHAZADA")).display()

# COMMAND ----------

fact_pedidos = (fact_pedidos
    .withColumn("ANIO_MES_PEDIDO", date_format(col("FECHA_PEDIDO"), "yyyy-MM"))
    .withColumn("IMPORTE_RECHAZADA_AJUSTADO", when(col("ANIO_MES_PEDIDO") == col("ANIO_MES"), col("IMPORTE_RECHAZADA")).otherwise(0))
    .withColumn("DEMANDA_MKT", col("IMPORTE") - col("IMPORTE_RECHAZADA_AJUSTADO"))
)


# COMMAND ----------

fact_pedidos.filter(col("ID_MATERIAL")=='MU-41420-E-D').groupBy("ANIO_MES","ANIO_MES_PEDIDO").agg(sum("IMPORTE"),sum("IMPORTE_RECHAZADA"),sum("IMPORTE_RECHAZADA_AJUSTADO"),sum("DEMANDA_MKT")).display()
...
fact_pedidos = (fact_pedidos.groupBy("ANIO_MES","FECHA_REGISTRO","MES_FECHA_REGISTRO","ANIO_FECHA_REGISTRO",
    ...
    sum("IMPORTE_BACKORDER_DOLARES").alias('IMPORTE_BACKORDER_DOLARES'),
    sum("IMPORTE_DOLARES").alias('IMPORTE_DOLARES'),
    sum("DEMANDA_MKT").alias('DEMANDA_MKT')
))
...
fact_pedidos.filter(col("ANIO_MES")=='2025-01').display()
...
# fact_pedidos = fact_pedidos.join(dim_productos, on = ["ID_MATERIAL","ID_CANAL_DISTRIBUCION"], how = 'inner')