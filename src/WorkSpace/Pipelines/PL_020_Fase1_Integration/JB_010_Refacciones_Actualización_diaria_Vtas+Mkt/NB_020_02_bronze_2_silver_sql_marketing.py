...
#display(fact_servicio.filter(col("ANIO_MES") == "2025-12")) #PERIODO
#display(fact_servicio_json.filter(col("PERIODO") >= "2025-11-10"))
...
#Selección de campos de tabla margen cliente producto
...
#Se agrupo por la máxima fecha de carga para cada anio_mes
agrupador_margen_cliente_producto = fact_margen_cliente_producto.groupBy("ANIO_MES").agg(max("FECCARGA").alias("FECCARGA"))

# COMMAND ----------

# agrupador_margen_cliente_producto.display()

# COMMAND ----------

#Se filtra por la maxima fecha de carga para cada anio_mes
...
#Se quitan 0 y vacios al inicio de los IDs
...
# MAGIC %md
# MAGIC #Nivel de servicio

# COMMAND ----------

#Seleccion campos de nivel de servicio
table = "FACT_IND_SD_FILLRATE"
fact_servicio = (spark.
                     read.
                     parquet(f"/mnt/adls-dac-data-bi-pr-scus/bronze/tablas_sql/{table}.parquet/"))                  
...
#Se borra las filas nulas
fact_servicio = fact_servicio.na.drop(how='all')


# COMMAND ----------

#Se agrupan las maximas fehca
